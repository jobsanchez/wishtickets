-- Revenue drilldown: manual distribution bookings were labeled paymongo/onsite only.
-- Show payment_method as distributed / complimentary when tied to admin_seat_assignments.
  ...
  CASE p_metric
    ...
    WHEN 'revenue' THEN
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'payment_method', CASE
          WHEN a.id IS NOT NULL AND a.distribution_category = 'sales' THEN 'distributed'
          WHEN a.id IS NOT NULL AND a.distribution_category = 'complementary' THEN 'complimentary'
          WHEN b.accepted_by_admin_id IS NULL THEN 'paymongo'
          ELSE 'onsite'
        END,
        'section_name', tm.section_name,
        'row_label', tm.row_label,
        'seat_number', tm.seat_number,
      ...
      LEFT JOIN LATERAL (
        SELECT id, recipient_name, recipient_email, distribution_category
        FROM public.admin_seat_assignments
      ...
      LEFT JOIN LATERAL (
        SELECT
          CASE
            WHEN COUNT(*) = 0 THEN '—'
            WHEN COUNT(DISTINCT COALESCE(NULLIF(TRIM(es_sec.name), ''), es_sec.section_code, sec.name, 'Other')) = 1
              THEN MIN(COALESCE(NULLIF(TRIM(es_sec.name), ''), es_sec.section_code, sec.name, 'Other'))
            ELSE 'Multiple sections'
          END AS section_name,
          CASE
            WHEN COUNT(*) = 0 THEN '—'
            WHEN COUNT(DISTINCT COALESCE(es.row_label, '—')) = 1
              THEN MIN(COALESCE(es.row_label, '—'))
            ELSE '—'
          END AS row_label,
          CASE
            WHEN COUNT(*) = 0 THEN 'General'
            WHEN COUNT(DISTINCT COALESCE(es.seat_number, 'General')) = 1
              THEN MIN(COALESCE(es.seat_number, 'General'))
            ELSE 'General'
          END AS seat_number,
          COALESCE(SUM(COALESCE(t.quantity, 1)), 0)::int AS quantity
        FROM public.tickets t
      ...
      WHERE b.event_id = p_event_id AND b.status = 'confirmed'
        AND EXISTS (
          SELECT 1
          FROM public.tickets t_exist
          WHERE t_exist.booking_id = b.id
        )
        AND NOT EXISTS (
          SELECT 1
    ...
    WHEN 'distributed' THEN
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'recipient_name', COALESCE(NULLIF(TRIM(sub.recipient_name), ''), '—'),
      ...
      FROM (
        SELECT t.id, a.id AS aid, es_sec.id AS es_sec_id, sec.id AS sec_id,
          ...
          COALESCE(NULLIF(TRIM(a.recipient_name), ''), NULLIF(TRIM(t.recipient_name), ''), p.full_name, '—') AS recipient_name,
          a.recipient_email,
          t.quantity
        FROM public.tickets t
        JOIN public.bookings b ON b.id = t.booking_id
    ...
    WHEN 'admitted' THEN
      -- One row per distinct ticket, first admission only. Include ticket_id and event_section_id for delete actions.
      SELECT jsonb_agg(jsonb_build_object(
        'section_name', section_name,
      ...
      FROM (
        SELECT DISTINCT ON (ar.ticket_id)
          ...
          ar.ticket_id AS ticket_id,
          es_sec.id AS event_section_id
