-- When a section has seat_layout_canvas_id, use the canvas image for background_image_url in get_event_availability
...
BEGIN
  ...
  IF v_use_event_seating THEN
    ...
    WITH sec_data AS (
      ...
      GROUP BY sec.id, sec.name, sec.section_code, sec.capacity, sec.sort_order, sec.seating_type, sec.color, sec.show_seat_selection, sec.seat_layout_image_url, sec.seat_layout_scale, sec.seat_layout_opacity, elc.image_url, elc.scale, elc.opacity
    )
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', id,
