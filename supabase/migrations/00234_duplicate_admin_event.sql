-- Duplicate an event as a configuration template (no bookings/tickets/etc.).
-- Omitted on purpose (audit when adding new FK to public.events): bookings, tickets,
-- payments, reservation_* , admin_seat_assignments, admission_records,
-- print_* job/ticket tables, shared_report_links, event_audit_logs, etc.
CREATE EXTENSION IF NOT EXISTS pgcrypto;
-- Copy: events row (draft), canvases, sections, seats (new scan_code + encrypted_qr),
--       prices, early_bird_prices (remapped sections), promo_codes (unique codes),
--       add-ons, banners, admissions codes (new gate codes), event_administrators.
-- When extending public.events template fields, extend the INSERT SELECT below.
