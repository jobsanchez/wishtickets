-- Chunked seat copy: phase1 = one txn (event + layout + pricing + …, no seats);
-- seats_chunk = separate txn per batch (smaller locks / statement work per commit).

CREATE TABLE IF NOT EXISTS public.event_duplicate_jobs (
  ...
  status text NOT NULL CHECK (status IN ('seats_pending', 'completed', 'failed')),
  last_source_seat_id uuid,
  error_message text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_event_duplicate_jobs_target
  ON public.event_duplicate_jobs (target_event_id);
CREATE INDEX IF NOT EXISTS idx_event_duplicate_jobs_source
  ON public.event_duplicate_jobs (source_event_id);

CREATE TABLE IF NOT EXISTS public.event_duplicate_section_map (
  job_id uuid NOT NULL REFERENCES public.event_duplicate_jobs(id) ON DELETE CASCADE,
  old_section_id uuid NOT NULL,
  new_section_id uuid NOT NULL,
  PRIMARY KEY (job_id, old_section_id)
);

CREATE INDEX IF NOT EXISTS idx_event_duplicate_section_map_new
  ON public.event_duplicate_section_map (new_section_id);

ALTER TABLE public.event_duplicate_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.event_duplicate_section_map ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "deny_all_event_duplicate_jobs" ON public.event_duplicate_jobs;
CREATE POLICY "deny_all_event_duplicate_jobs"
  ON public.event_duplicate_jobs FOR ALL TO authenticated USING (false) WITH CHECK (false);

DROP POLICY IF EXISTS "deny_all_event_duplicate_section_map" ON public.event_duplicate_section_map;
CREATE POLICY "deny_all_event_duplicate_section_map"
  ON public.event_duplicate_section_map FOR ALL TO authenticated USING (false) WITH CHECK (false);

DROP FUNCTION IF EXISTS public.duplicate_admin_event(uuid, text);

CREATE OR REPLACE FUNCTION public.duplicate_admin_event_phase1(p_source_id uuid, p_new_title text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.event_duplicate_jobs (
    id,
    source_event_id,
    target_event_id,
    created_by,
  ...
  INSERT INTO public.event_duplicate_section_map (job_id, old_section_id, new_section_id)
  SELECT v_job_id, dm.old_id, dm.new_id
  ...
  RETURN jsonb_build_object(
    'job_id', v_job_id,
    'target_event_id', v_new_id
  );
END;
$$;

CREATE OR REPLACE FUNCTION public.duplicate_admin_event_seats_chunk(
  p_job_id uuid,
  p_batch_size int DEFAULT 1500
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_job public.event_duplicate_jobs%ROWTYPE;
  v_source uuid;
  v_target uuid;
  v_evt_code text;
  v_batch int;
  ...
  v_enc text;
  v_copied int := 0;
  v_max_source_seat_id uuid;
BEGIN
  v_batch := greatest(1, least(coalesce(p_batch_size, 1500), 5000));

  SELECT * INTO v_job FROM public.event_duplicate_jobs j WHERE j.id = p_job_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Duplicate job not found';
  END IF;

  IF NOT public.is_authorized_for_event(v_job.source_event_id) THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;

  IF v_job.status <> 'seats_pending' THEN
    RETURN jsonb_build_object('copied', 0, 'done', true);
  END IF;

  v_source := v_job.source_event_id;
  v_target := v_job.target_event_id;

  SELECT e.event_code INTO STRICT v_evt_code FROM public.events e WHERE e.id = v_target;

  v_max_source_seat_id := NULL;

  FOR seat_row IN
    SELECT src_seat.*
    FROM public.event_seats src_seat
    WHERE src_seat.event_id = v_source
      AND (
        v_job.last_source_seat_id IS NULL
        OR src_seat.id > v_job.last_source_seat_id
      )
    ORDER BY src_seat.id ASC
    LIMIT v_batch
  LOOP
    SELECT sm.new_section_id INTO v_new_sec_id
    FROM public.event_duplicate_section_map sm
    WHERE sm.job_id = p_job_id
      AND sm.old_section_id = seat_row.event_section_id
    LIMIT 1;
    ...
    LOOP
      ...
      EXIT WHEN NOT EXISTS (
        ...
        WHERE x.event_id = v_target AND x.scan_code = v_scan
      );
    END LOOP;
        ...
        digest(
          upper(btrim(
            substring(upper(coalesce(v_evt_code, 'XXX')), 1, 3)
              ...
              || coalesce(seat_row.row_label::text, '-')
              || coalesce(seat_row.seat_number::text, '-')
          )),
          'sha256'::text
        ),
        'hex'::text
    ...
    v_copied := v_copied + 1;
    v_max_source_seat_id := seat_row.id;
  END LOOP;

  IF v_copied = 0 THEN
    UPDATE public.event_duplicate_jobs
    SET status = 'completed', updated_at = now(), last_source_seat_id = NULL
    WHERE id = p_job_id;
    DELETE FROM public.event_duplicate_section_map WHERE job_id = p_job_id;
    RETURN jsonb_build_object('copied', 0, 'done', true);
  END IF;

  UPDATE public.event_duplicate_jobs
  SET
    last_source_seat_id = v_max_source_seat_id,
    updated_at = now()
  WHERE id = p_job_id;

  IF v_copied < v_batch THEN
    UPDATE public.event_duplicate_jobs
    SET status = 'completed', updated_at = now(), last_source_seat_id = NULL
    WHERE id = p_job_id;
    DELETE FROM public.event_duplicate_section_map WHERE job_id = p_job_id;
    RETURN jsonb_build_object('copied', v_copied, 'done', true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.event_seats es2
    WHERE es2.event_id = v_source
      AND es2.id > v_max_source_seat_id
  ) THEN
    UPDATE public.event_duplicate_jobs
    SET status = 'completed', updated_at = now(), last_source_seat_id = NULL
    WHERE id = p_job_id;
  ...
  RETURN jsonb_build_object('copied', v_copied, 'done', false);
END;
$$;

GRANT EXECUTE ON FUNCTION public.duplicate_admin_event_phase1(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.duplicate_admin_event_seats_chunk(uuid, int) TO authenticated;
