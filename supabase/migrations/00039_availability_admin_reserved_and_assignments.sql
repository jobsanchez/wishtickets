-- 1. Update get_event_availability: deduct admin_assignment_items (reserved) from section availability
-- 2. Update get_admin_seat_assignments: union event_seats + admin_assignment_items
  ...
  IF v_use_event_seating THEN
    ...
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'available', GREATEST(0, capacity - booked_qty - reserved_qty - admin_reserved_qty)
      )
...
-- Update get_admin_seat_assignments: union event_seats + admin_assignment_items
    ...
    (
      ...
      FROM (
        ...
        SELECT 2 AS ord, jsonb_build_object(
          ...
          'seat_label', 'Section x' || ai.quantity
        ) AS item
        FROM public.admin_assignment_items ai
