-- Include section color in get_event_availability for buyer seat display
...
BEGIN
  ...
  IF v_use_event_seating THEN
    ...
    WITH sec_data AS (
      ...
      GROUP BY sec.id, sec.name, sec.section_code, sec.capacity, sec.sort_order, sec.seating_type, sec.color
    )
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', id,
