-- Split public availability into manifest (sections + canvases) and seats payloads so HTTP can
-- load lighter JSON first then seat rows (optional section filter). Shorter Lambda work per request.

CREATE OR REPLACE FUNCTION public.get_event_availability_manifest(p_event_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.get_event_availability_seats(
  p_event_id uuid,
  p_section_ids uuid[]
)
RETURNS jsonb
...
BEGIN
  ...
  v_all_sections := (p_section_ids IS NULL OR array_length(p_section_ids, 1) IS NULL);
  ...
  IF v_use_event_seating THEN
    ...
    WHERE es.event_id = p_event_id
      AND (
        v_all_sections
        OR es.event_section_id = ANY(p_section_ids)
      );
  ELSE
    SELECT jsonb_agg(
      jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.get_event_availability_manifest(uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_event_availability_seats(uuid, uuid[]) TO anon, authenticated;
