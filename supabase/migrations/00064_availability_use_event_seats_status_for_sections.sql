-- Use event_seats.status as source of truth for section availability when section has event_seats.
-- Fixes Sold Out banner not showing when event_seats are marked sold.
...
BEGIN
  ...
  IF v_use_event_seating THEN
    ...
    WITH sec_data AS (
      ...
      GROUP BY sec.id, sec.name, sec.section_code, sec.capacity, sec.sort_order, sec.seating_type, sec.color, sec.show_seat_selection
    )
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', id,
