-- Events can use event_sections / event_seats without a linked venue_id.
-- Previously, get_event_availability returned NULL whenever venue_id was NULL, so buyers saw
-- empty availability even when seating was fully configured on the event.
