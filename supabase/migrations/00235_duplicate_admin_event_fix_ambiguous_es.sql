-- Hotfix duplicate_admin_event PL/pgSQL name clashes in the seat loop query:
-- (1) FOR iterator `es` must not share alias with `event_seats es` (use `src_seat`).
-- (2) DECLARE `s record` must not share alias `s` on `event_seats` — same `src_seat` fix.
-- Databases that already applied an older 00235 may need 00236 as well.
