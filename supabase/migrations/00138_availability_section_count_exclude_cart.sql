-- Fix buyer availability count: exclude cart-held seats from "remaining", align with admin view.
-- Before: available_from_seats = COUNT(status='available') — cart-held seats stay available, so counted.
-- After: available_from_seats = COUNT(seats NOT IN booked OR reserved) — same logic as per-seat and admin.
