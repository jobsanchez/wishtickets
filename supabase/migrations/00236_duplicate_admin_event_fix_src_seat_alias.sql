-- Hotfix: DECLARE `s record` conflicted with SQL alias `s` on event_seats (`s.*` ambiguous). Use alias `src_seat`. Replaces function for DBs that applied 00235 with `SELECT s.* FROM event_seats s`.
