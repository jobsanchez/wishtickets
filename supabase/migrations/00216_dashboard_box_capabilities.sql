-- Dashboard card capabilities (refund lookup, ticket resending) and RPC whitelist.
-- Backfill: existing accounts with role = admin get full dashboard box caps to match prior "admin sees all" behavior.
...
INSERT INTO public.user_capabilities (user_id, capability)
SELECT user_id, 'resend_tickets'
FROM public.user_capabilities
WHERE capability = 'view_sales_analytics'
ON CONFLICT (user_id, capability) DO NOTHING;
